...
for($i=0; $i < count($routines); $i++){
  ...
  for($j=0; $j < count($actions); $j++){
	...
	for($k=0; $k < count($hosts); $k++){
	  $query = "SELECT ti.TestResult_Value, DATE_FORMAT(ti.TestInstance_Date,'%m/%d') FROM TestInstanceBest ti WHERE Host='$hosts[$k]' AND DatasetName='$instances[$l]' AND LibraryVersion='1.6' AND TestInstance_Date BETWEEN '$start_date' AND '$end_date' ORDER BY TestInstance_Date DESC";
	...
	$graph[$i][$j]->SetShadow();
	$graph[$i][$j]->footer->center->Set("HDF5 1.6");
