...
TEMP="/usr/local/home/hyoklee/chicago/tmp/chicago_$VERSION"
# cc or gcc version command
CCV="gcc -v"
# CC or g++ version command
CPPV="g++ -v"
# HDF5 Installation Directory
HDF5_PREFIX="/usr/local/home/hyoklee/chicago/hdf5-$VERSION"
...
PERF_OPTION="--disable-shared --prefix=/usr/local/home/hyoklee/chicago --with-hdf5=$HDF5_PREFIX --with-mysqlclient=/hdfdap/mysql"
# Path to performance framework source
PERF_SRC="/usr/local/home/hyoklee/src/chicago/trunk/hdf5perflib/"
# Path to php command
# e.g. PHP=/opt/csw/bin/php
PHP="php"
# Path to performance framework php source 
PHP_SRC="/usr/local/home/hyoklee/src/chicago/trunk/hdf5perfphp/hdf/"
...
./configure $HDF5_OPTION | grep -v '^checking' | grep -v '^config.status' | grep -v '^configure:' | grep -v '^appending configuration' >  $TEMP/compiler_options_hdf5.txt
$MAKE
$MAKE install
cd $PERF_SRC
