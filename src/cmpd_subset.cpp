 ...
 *
 * \author Hyo-Kyung (Joe) Lee (hyolee@hdfgroup.org)
 * \date March 18, 2021
 * \note added Catch-2 test.
 *
...
create_stype1(void)
{
    ...
    if((array_dt1 = H5Tarray_create(H5T_NATIVE_INT,1, &eight))<0)
        goto error;
    if((array_dt2 = H5Tarray_create(H5T_NATIVE_FLOAT,1, &sixteen))<0)
        goto error;
 ...
 */
static int
test_hdf5_src_subset(const char *filename, double *time_used)
{
    hid_t   file;     
    ...
    /* Rewrite chunked data set */
      // Version 1.6 way
    if((dataset = H5Dopen(file, DSET_NAME[1], NULL))<0)
 ...
 */
static int
test_hdf5_dst_subset(const char *filename, double *time_used)
{
    hid_t   file;     
    hid_t   rew_tid, src_tid, dst_tid;
    hid_t   dataset;
    ...
    /* Rewrite chunked data set */
    if((dataset = H5Dopen(file, DSET_NAME[3], NULL))<0)
        goto error;
...
TEST_CASE("cmpd_subset") {
  // now let's benchmark:
  BENCHMARK("cmpd_subset 100") {
      test();
  };
}
