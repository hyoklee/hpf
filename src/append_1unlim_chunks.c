...
const char *FILENAME[] = {
    "old_append_1unlim.h5",
    "new_append_1unlim.h5",
    NULL
};
...
/* Dataset names */
#define XDSET_1UNLIM    "x_dset_1unlim"		/* Dataset for appending along the x direction */
#define YDSET_1UNLIM    "y_dset_1unlim"		/* Dataset for appending along the y direction */

/* Dataset expansion direction */
typedef enum expand_dir_t {
    xdir,	/* X direction */
    ydir,	/* Y direction */
    xydir	/* XY direction: diagonally */
} expand_dir_t;

/*
 * Create the test file with old or new format.
 * Create 2 datasets in the file:
 *  (a) XDSET_1UNLIM: 2-d dataset with 1 unlimited dim; append along the x direction
 *  (b) YDSET_1UNLIM: 2-d dataset with 1 unlimited dim; append along the y direction
 *
 * Old format: will use BT1 chunk indexing method
 * New format: will use EA chunk indexing method
 */
test_setup(const char *fname, hbool_t newformat)
{
    ...
    /* Create 2 datasets: XDSET_1UNLIM, YDSET_1UNLIM  */
    ...
    /* Create XDSET_1UNLIM: 1unlim, xdir */
    ...
    /* Create the dataset */
    if((did = H5Dcreate(fid, XDSET_1UNLIM, H5T_NATIVE_UCHAR, sid, dcpl)) < 0)
        TEST_ERROR
    ...
    /* Create YDSET_1UNLIM: 1unlim, ydir */
    ...
    /* Create the dataset */
    if((did = H5Dcreate(fid, YDSET_1UNLIM, H5T_NATIVE_UCHAR, sid, dcpl)) < 0)
        TEST_ERROR
 ...
 *
 * Purpose:     Measure the time in appending 1-byte chunk up to NELEMENTS to the dataset.
 *
 * Return:      Success:        0
 *              Failure:        1
 *-------------------------------------------------------------------------
...
append_dset(const char *filename, hbool_t newformat, char *dname, expand_dir_t dir, double *time_used)
{
    ...
    strcpy(mesg, "Append chunks to 2d dataset with 1 unlimited dimension for");
    if(newformat)
 ...
 *-------------------------------------------------------------------------
 */
int database(double old_xtime, double old_ytime, double new_xtime, double new_ytime)
{
    time_t clock1;
    struct tm *t1;
    ...
    find_routine = H5Perf_find_routine(filestorage_handle,"append to 2d dset - 1-unlim");
    if(find_routine >= 0) {
        routine_handle = find_routine;

        find_action1 = H5Perf_find_action(routine_handle,"append to 2d dset - 1-unlim-old-x");
        if(find_action1 <0) {
            if (H5Perf_addAction(routine_handle,
				 "append to 2d dset - 1-unlim-old-x",
				 "measure time for appending to 2d dset - 1-unlim-old-x",
				 env_action) < 0){
        ...
        find_action2 = H5Perf_find_action(routine_handle,"append to 2d dset - 1-unlim-old-y");
        if(find_action2 <0) {
            if (H5Perf_addAction(routine_handle,
				 "append to 2d dset - 1-unlim-old-y",
				 "measure time for appending to 2d dset - 1-unlim-old-y",
				 env_action) < 0){
                printf("Unable to add action to test routine object at line %d\n",__LINE__);
                H5Perf_end();
                goto error;
        ...
        find_action3 = H5Perf_find_action(routine_handle,
					  "append to 2d dset - 1-unlim-new-x");
        if(find_action3 <0) {
            if (H5Perf_addAction(routine_handle,
				 "append to 2d dset - 1-unlim-new-x",
				 "measure time for appending to 2d dset - 1-unlim-new-x",
				 env_action) < 0){
                printf("Unable to add action to test routine object at line %d\n",__LINE__);
                H5Perf_end();
                goto error;
        ...
        find_action3 = H5Perf_find_action(routine_handle,
					  "append to 2d dset - 1-unlim-new-y");
        if(find_action3 <0) {
            if (H5Perf_addAction(routine_handle,
				 "append to 2d dset - 1-unlim-new-y",
				 "measure time for appending to 2d dset - 1-unlim-new-y",
				 env_action) < 0){
                printf("Unable to add action to test routine object at line %d\n",__LINE__);
                H5Perf_end();
                goto error;
        ...
        if(H5Perf_setRoutine(routine_handle, "append to 2d dset - 1-unlim",
			     "performance benchmark for appending to 2d dset - 1-unlim",
			     "1.0",env_action) <0) {
...
main(void)
{
    ...
    puts("Testing performance of chunk indexing methods-BT1 and EA:");
    if(test_setup(FILENAME[0], FALSE) < 0)
	TEST_ERROR
    if(test_setup(FILENAME[1], TRUE) < 0)
	TEST_ERROR

    nerrors += append_dset(FILENAME[0], FALSE, XDSET_1UNLIM, xdir, &old_xtime);
    nerrors += append_dset(FILENAME[0], FALSE, YDSET_1UNLIM, ydir, &old_ytime);
    nerrors += append_dset(FILENAME[1], TRUE, XDSET_1UNLIM, xdir, &new_xtime);
    nerrors += append_dset(FILENAME[1], TRUE, YDSET_1UNLIM, ydir, &new_ytime);

    /* Save the time into the database */
    nerrors += database(old_xtime, old_ytime, new_xtime, new_ytime);
